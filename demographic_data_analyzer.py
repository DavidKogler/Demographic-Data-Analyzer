...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = ((higher_education & (df['salary'] == ">50K")).sum() / higher_education.sum() * 100).round(1)
    lower_education_rich = ((lower_education & (df['salary'] == ">50K")).sum() / lower_education.sum() * 100).round(1)
    ...
    rich_percentage = (((df['hours-per-week'] == min_work_hours) & (df['salary'] == ">50K")).sum() / num_min_workers * 100).round(1)
    ...
    highest_earning_country_percentage = (((df['native-country'] == highest_earning_country) & (df['salary'] == ">50K")).sum() / (df['native-country'] == highest_earning_country).sum() * 100).round(1)
